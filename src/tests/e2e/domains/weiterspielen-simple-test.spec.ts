...
test.describe('Weiterspielen Simple Test', () => {
  ...
  test.skip('Train/1 → Kd5 → Weiterspielen → Schwarz macht keinen Zug (BUG)', async ({ page }) => {
    ...
    if (!moveSuccessful) {
      logger.error('❌ PROBLEM: Kf5 wurde nicht ausgeführt!');
      return;
    }
    ...
    if (schwarzHatGezogen) {
      logger.info('🎉 ÜBERRASCHUNG: Schwarz HAT einen Zug gemacht! Bug ist weg?');
      expect(schwarzHatGezogen).toBe(true);
    } else {
      logger.info('🐛 BUG BESTÄTIGT: Schwarz hat KEINEN Zug gemacht nach Weiterspielen');
      logger.info('Beweis:');
      logger.info(
        `- Move Count gleich: ${gameStateVorWeiterspielen.moveCount} → ${gameStateNachWarten.moveCount}`
      );
      logger.info('- FEN gleich:', gameStateVorWeiterspielen.fen === gameStateNachWarten.fen);
      logger.info('- Turn gleich:', gameStateVorWeiterspielen.turn === gameStateNachWarten.turn);

      // Dokumentiere den Bug (diese Assertion wird failsieren wenn Bug gefixt ist)
      expect(gameStateNachWarten.moveCount).toBe(gameStateVorWeiterspielen.moveCount);
