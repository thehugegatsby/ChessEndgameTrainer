// Activated from actual-position-test.skip.ts - test real position functionality
/**
...
test.describe('Actual Position 1 - King and Pawn vs King', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('requestfailed', request => {
      logger.error(`Network Request Failed: ${request.url()} - ${request.failure()?.errorText}`);
    });

    // Mock Lichess Tablebase API responses
    await page.route('https://tablebase.lichess.ovh/standard*', async route => {
  ...
  test.skip('should handle moves in the actual K+P position', async ({ page }) => {
    ...
    // TODO: Fix E2E interaction issues
    await test.step('Load Position 1 and verify', async () => {
      ...
      await expect(page.getByText('Opposition Grundlagen').first()).toBeVisible();
  ...
  test.skip('should show move history and evaluation', async ({ page }) => {
    ...
    // Wait for any tablebase analysis UI to appear - try different selectors
    ...
    for (const selector of possibleSelectors) {
      ...
      if (count > 0) {
        logger.info(`Found panel with selector: ${selector}`);
        panelFound = true;
        break;
      }
    }
    ...
    if (!panelFound) {
      logger.info('No specific tablebase panel found - checking for move display instead');
      // Alternative: Just check if we have any move-related content
      const moveContent = page.locator('text=/Kd6|Kf6|e6/');
      await expect(moveContent.first()).toBeVisible({ timeout: 5000 });
      logger.info('Found move content without specific panel');
    }
