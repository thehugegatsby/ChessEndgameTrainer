...
test.describe('Chess Move Scenarios', () => {
  ...
  test.skip('Promotion to Win - Auto-completion should trigger', async ({ page }) => {
    const runner = new SequenceRunner(page);
    await runner.executeSequence(promotionScenarios.promotionToWin);
  });

  test.skip('Promotion to Draw - Should NOT auto-complete', async ({ page }) => {
    const runner = new SequenceRunner(page);
  ...
  test.skip('Debug: Show current state after failed sequence', async ({ page }) => {
    const runner = new SequenceRunner(page);
