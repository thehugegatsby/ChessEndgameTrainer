...
test.describe('Phase 3 Error Scenarios with New Infrastructure', () => {
  ...
  test('should handle network timeout gracefully', async ({ page }) => {
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error('Expected timeout')), 5000)),
      ]);
  ...
  test('should handle API 500 errors with retry option', async ({ page }) => {
    ...
    const retryButton = page
      .locator('button:has-text("Wiederholen"), button:has-text("Retry")')
      .first();
  ...
  test('should recover from tablebase unavailable scenario', async ({ page }) => {
    ...
    const refreshButton = page
      .locator('button:has-text("Aktualisieren"), button:has-text("Refresh")')
      .first();
