...
test.describe.skip('Weiterspielen Bug Proof', () => {
  ...
  test('FIXED: Opponent DOES move after Weiterspielen', async ({ page }) => {
    ...
    const opponentMadeMove =
      gameStateAfterWaiting.moveCount > gameStateBeforeWeiterspielenClick.moveCount;

    if (opponentMadeMove) {
      logger.info('✅ SUCCESS: Opponent DID make a move! Bug is FIXED!');
      logger.info(
      ...
      logger.info(
        `- FEN unchanged: ${gameStateBeforeWeiterspielenClick.fen === gameStateAfterWaiting.fen}`
      );
      logger.info(
    ...
    expect(gameStateAfterWaiting.fen).not.toBe(gameStateBeforeWeiterspielenClick.fen);

    logger.info('✅ TEST COMPLETE: Bug is FIXED - opponent DOES move after Weiterspielen');
  });

  test('CONTROL: Verify that Zurücknehmen works correctly', async ({ page }) => {
    ...
    logger.info('✅ CONTROL TEST PASSED: Zurücknehmen correctly undoes the move');
  });
});
