...
test.describe.skip(
  'Pawn Promotion Simple (SequenceRunner) - Ready to unskip: Issue #99 resolved',
  () => {
    ...
    test('should handle multiple moves efficiently', async ({ page }) => {
      ...
      await runner.executeSequence({
        ...
        moves: ['e6-d6', 'e8-d8', 'd6-c6', 'd8-e7'],
        expectations: [expectation.storeState('game.moveCount', 4)],
        setup: {
          mockTablebase: true,
        },
      });
...
test.describe.skip(
  'SequenceRunner Migration Comparison - Ready to unskip: Issue #99 resolved',
  () => {
    ...
    test('Compare old vs new test approach', async ({ page }) => {
      ...
      const testScenario = {
        ...
        expectations: [expectation.storeState('game.moveCount', 5)],
