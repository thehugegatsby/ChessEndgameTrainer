...
class LRUCache<T> {
  ...
  constructor(
    maxSize: number = CACHE_DEFAULT_MAX_SIZE,
    ttlMinutes: number = CACHE_DEFAULT_TTL_MINUTES
  ) {
...
export class TablebaseService implements TablebaseServiceInterface {
  ...
  constructor() {
    ...
    this.evaluationCache = new LRUCache<TablebaseEvaluation>(
      CACHE_EVALUATIONS_SIZE,
  ...
  async evaluate(fen: string): Promise<TablebaseEvaluation> {
    ...
    try {
      ...
      const evaluation = tablebaseTransformer.normalizePositionEvaluation(apiResponse, fen);
   ...
   */
  private transformMoves(apiResponse: TablebaseApiResponse, fen: string): TablebaseMove[] {
  ...
  private handleError(error: unknown): TablebaseError {
    ...
    if (error instanceof ApiError) {
      ...
      if (error.status === HTTP_NOT_FOUND || error.code === 'NOT_FOUND') {
        return new TablebaseError('Position not in tablebase', 'NOT_FOUND');
      }

      if (error.code === 'TIMEOUT') {
        return new TablebaseError('Tablebase service temporarily unavailable', 'UNAVAILABLE');
      }

      return new TablebaseError(error.message, 'API_ERROR');
    }

    // Validation errors
    if (error instanceof Error && error.message.includes('FEN')) {
      return new TablebaseError(error.message, 'INVALID_FEN');
    }
    ...
    // Unknown errors
    return new TablebaseError('An unknown error occurred', 'API_ERROR');
  }
  ...
  async prefetch(fens: string[]): Promise<void> {
    ...
    const results = await Promise.allSettled(fens.map(fen => this.evaluate(fen)));
