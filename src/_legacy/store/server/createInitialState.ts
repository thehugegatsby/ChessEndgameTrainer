...
import type { TrainingPosition } from './store/slices/trainingSlice';
 ...
 */
function toTrainingPosition(
  position: EndgamePosition & Partial<TrainingPosition>
): TrainingPosition {
  return {
    ...position,
    colorToTrain: position.colorToTrain || position.sideToMove || 'white',
    targetOutcome:
      position.targetOutcome ||
      (() => {
...
export async function createInitialStateForPosition(
  ...
  const trainingPosition = toTrainingPosition(
    position as EndgamePosition & Partial<TrainingPosition>
  );
  ...
  const initialState = {
    ...
    game: {
      ...
      currentPgn: '', // Will be generated as needed
