...
import { type PositionRepository, type PositionRepositoryConfig } from '../IPositionRepository';
  ...
  async getNextPosition(currentId: number, categoryId?: string): Promise<EndgamePosition | null> {
    try {
      ...
      if (categoryId) {
        ...
        q = query(positionsRef, where('id', '>', currentId), orderBy('id'), limit(1));
      }
  ...
  async getPreviousPosition(
    ...
    try {
      ...
      if (categoryId) {
        ...
        q = query(positionsRef, where('id', '<', currentId), orderBy('id', 'desc'), limit(1));
      }
