...
async function runComparison() {
  ...
  console.log(`   sql.js init: ${sqlJsResults.initTime?.toFixed(2) || 'N/A'}ms`);
  console.log(`   wa-sqlite init: ${waSqliteResults.initTime}ms (estimated)`);
  console.log(`   sql.js insert: ${sqlJsResults.insertTime?.toFixed(2) || 'N/A'}ms`);
  console.log(`   wa-sqlite insert: ${waSqliteResults.insertTime}ms (estimated)`);
  ...
  const criteria = {
    ...
    performance: { weight: 0.30, sqlJs: 7, waSqlite: 9 },
    stability: { weight: 0.25, sqlJs: 9, waSqlite: 6 },
    compatibility: { weight: 0.20, sqlJs: 10, waSqlite: 6 }
  };
