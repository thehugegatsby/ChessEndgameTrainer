...
async function renamePositionIds() {
  ...
  try {
    ...
    for (const [oldId, newId] of renameMap) {
      ...
      console.log(
        `🔍 Checking source position ${oldId}: exists = ${sourceSnap.exists()}`,
      );
      ...
      console.log(
        `🔍 Checking target position ${newId}: exists = ${targetSnap.exists()}`,
      );

      if (targetSnap.exists()) {
        console.warn(
          `⚠️  Position ${newId} already exists, skipping rename from ${oldId}`,
        );
      } else {
