...
try {
  ...
  console.log(
    "   firebase firestore:import --project chess-endgame-trainer-c1ea6 firestore_import_data --overwrite",
  );
} catch (error) {
  console.error("❌ Error preparing data:", error.message);
  if (error.code === "ENOENT") {
    console.error(
      `Please ensure '${inputFileName}' exists in the same directory as the script.`,
    );
