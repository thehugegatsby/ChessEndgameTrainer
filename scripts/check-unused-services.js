...
function checkServiceUsage(servicePath, serviceName) {
  ...
  const usageReport = files.map(file => {
    ...
    try {
      ...
      const importingFiles = importSearch.trim().split('\n').filter(f => f);
...
function checkUnusedComponents() {
  ...
  for (const component of componentsToCheck) {
    try {
      ...
      const usageCount = usage.trim().split('\n').filter(l => l).length;
...
console.log(`🟡 Services with unused files: ${report.services.filter(s => s.unusedFiles.length > 0).length}`);
