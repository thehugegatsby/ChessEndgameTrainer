...
const firebaseConfig = {
  apiKey:
    ...
    "AIzaSyAtMncqDwhbZtbIjYjTlL1ViKqW3sJSHjs",
  authDomain:
    process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN ||
    "chess-endgame-trainer-c1ea6.firebaseapp.com",
  projectId:
    process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID ||
    "chess-endgame-trainer-c1ea6",
  storageBucket:
    process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET ||
    "chess-endgame-trainer-c1ea6.firebasestorage.app",
  messagingSenderId:
    process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID || "884956836859",
  appId:
...
async function importCollection(collectionName, collectionPath) {
  ...
  for (const file of files) {
    if (path.extname(file) === ".json") {
      ...
      if (batchCount === BATCH_SIZE) {
        console.log(
          `📦 Committing batch of ${batchCount} documents for ${collectionName}...`,
        );
        try {
          await batch.commit();
          totalDocsImported += batchCount;
          console.log(
            `✅ Batch committed successfully. Total imported: ${totalDocsImported}`,
          );
          batch = writeBatch(db);
          batchCount = 0;
        } catch (error) {
          console.error(
            `❌ Failed to commit batch for ${collectionName}:`,
            error,
          );
          batch = writeBatch(db);
          batchCount = 0;
  ...
  if (batchCount > 0) {
    console.log(
      `📦 Committing final batch of ${batchCount} documents for ${collectionName}...`,
    );
    try {
      await batch.commit();
      totalDocsImported += batchCount;
      ...
      console.error(
        `❌ Failed to commit final batch for ${collectionName}:`,
        error,
      );
    }
  }
  ...
  console.log(
    `   📊 Total processed: ${totalDocsProcessed}, Successfully imported: ${totalDocsImported}`,
  );
}
