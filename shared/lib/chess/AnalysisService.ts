...
export class AnalysisService {
  ...
  async getBestMove(fen: string): Promise<string | null> {
    logger.info('[AnalysisService] Getting best move');
    ...
    try {
      ...
      if (tablebaseMoves.isAvailable && tablebaseMoves.moves && tablebaseMoves.moves.length > 0) {
        logger.info('[AnalysisService] Using tablebase move', { move: tablebaseMoves.moves[0].san });
        return tablebaseMoves.moves[0].san;
      }
