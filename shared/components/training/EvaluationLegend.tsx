...
import { TABLEBASE_LEGEND, ENGINE_LEGEND } from '../../utils/chess/evaluationHelpers';
...
export const EvaluationLegend: React.FC<EvaluationLegendProps> = ({ className = '' }) => {
  ...
  return (
    <div className={`${className}`}>
      ...
      {isExpanded && (
        <div className="mt-2 p-3 rounded text-xs space-y-3"
             style={{ 
               backgroundColor: 'var(--bg-tertiary)',
          ...
          {/* Engine Legend */}
          <div>
            <h4 className="font-medium mb-2 text-blue-400">🤖 Engine (Schätzungen)</h4>
            <div className="grid grid-cols-1 gap-1">
              {Object.entries(ENGINE_LEGEND).map(([symbol, description]) => (
                <div key={symbol} className="flex items-center gap-2">
                  <span className="w-6 text-center">{symbol}</span>
                  <span style={{ color: 'var(--text-secondary)' }}>{description}</span>
                </div>
