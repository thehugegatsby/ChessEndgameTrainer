...
export const useTrainingGame = ({ 
  ...
  const makeMove = useCallback(async (move: { from: string; to: string; promotion?: string }): Promise<boolean> => {
    ...
    try {
      ...
      actions.makeMove(move as any);
      
      // Check if game is finished after move
      if (training.game?.isGameOver()) {
        const success = training.game.turn() !== training.currentPosition?.sideToMove?.[0];
  ...
  const undoMove = useCallback((): boolean => {
    ...
    return true;
  }, [training.moveHistory, training.currentFen, training.currentPgn, actions, onPositionChange]);
