...
export class FirestoreDebugPage extends BasePage {
   ...
   */
  async clearAllData(options: { confirm?: boolean; waitForComplete?: boolean } = {}): Promise<void> {
    ...
    if (confirm) {
      const confirmButton = this.page.locator('button:has-text("Confirm"), button:has-text("Clear"), button:has-text("Delete")');
  ...
  async selectPosition(title: string): Promise<void> {
    ...
    await this.page.waitForSelector('[data-testid="position-details"]', { state: 'visible', timeout: 5000 });
  }
