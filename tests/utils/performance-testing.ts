...
export class PerformanceTester {
  ...
  async assertBudget(budget: PerformanceBudget): Promise<void> {
    ...
    if (budget.maxResourceSize && metrics.totalResourceSize > budget.maxResourceSize) {
      throw new Error(`Resource size ${metrics.totalResourceSize} exceeds budget ${budget.maxResourceSize}`);
    }
    
    if (budget.maxJSHeapSize && metrics.jsHeapUsedSize && metrics.jsHeapUsedSize > budget.maxJSHeapSize) {
      throw new Error(`JS heap size ${metrics.jsHeapUsedSize} exceeds budget ${budget.maxJSHeapSize}`);
    }
  }
  ...
  static async userJourney(
    ...
    for (const step of journey.steps) {
      ...
      if (step.budget && duration > step.budget) {
        throw new Error(`Step "${step.name}" took ${duration}ms, exceeding budget of ${step.budget}ms`);
      }
    }
