/**
 * @fileoverview Generic E2E Test for Chess Move Scenarios
 * 
...
test.describe("Chess Move Scenarios", () => {
  ...
  test("Promotion to Win - Auto-completion should trigger", async ({ page }) => {
    const runner = new SequenceRunner(page);
    await runner.executeSequence(promotionScenarios.promotionToWin);
  });

  test("Promotion to Draw - Should NOT auto-complete", async ({ page }) => {
    const runner = new SequenceRunner(page);
    await runner.executeSequence(promotionScenarios.promotionToDraw);
  });

  test("Promotion to Queen - Correct German message", async ({ page }) => {
    const runner = new SequenceRunner(page);
    await runner.executeSequence(promotionScenarios.promotionToQueen);
  });

  test("Promotion to Rook - Correct German message", async ({ page }) => {
    const runner = new SequenceRunner(page);
    await runner.executeSequence(promotionScenarios.promotionToRook);
  });

  test("Debug: Show current state after failed sequence", async ({ page }) => {
    const runner = new SequenceRunner(page);
    ...
    try {
      ...
      console.log("Store state:", JSON.stringify(storeState.ui?.toasts, null, 2));
      console.log("Training state:", JSON.stringify(storeState.training, null, 2));
