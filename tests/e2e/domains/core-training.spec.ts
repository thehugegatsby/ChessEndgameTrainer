...
test.describe('Core Training Workflow', () => {
  ...
  test('should complete homepage → training → move → evaluation journey', async ({ page }) => {
    ...
    await expect(page.locator(E2E.SELECTORS.BOARD)
    ...
    const evaluation = page.locator(E2E.SELECTORS.EVALUATION_DISPLAY)
    ...
    if (await engineError.isVisible()) {
      ...
      if (errorText && !errorText.includes('this.eventEmitter.emit') && !errorText.includes('const error')) {
  ...
  test('should handle engine initialization', async ({ page }) => {
    ...
    logger.info(E2E.MESSAGES.SUCCESS.ENGINE_VERIFIED);
  });
});