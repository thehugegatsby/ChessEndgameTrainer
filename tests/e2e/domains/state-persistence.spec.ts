...
test.describe('State Persistence - Issue #23', () => {
  ...
  test('should persist and restore game state after page reload', async ({ page }) => {
    ...
    await page.evaluate((state) => {
      localStorage.setItem('chess-trainer-storage', JSON.stringify({
        state,
        version: 0
      }));
  ...
  test('should preserve position navigation state across reloads', async ({ page }) => {
    ...
    logger.info(E2E.MESSAGES.SUCCESS.POSITION_PRESERVED, { originalUrl, restoredUrl });
  });
  ...
  test('should handle localStorage corruption gracefully', async ({ page }) => {
    ...
    logger.info(E2E.MESSAGES.SUCCESS.CORRUPTION_HANDLED, { pageContent, hasError });
  });
  ...
  test('should preserve user preferences across sessions', async ({ page }) => {
    ...
    expect(restoredState?.user?.preferences?.theme).toBe(E2E.DATA.PREFERENCES.theme);
    expect(restoredState?.user?.preferences?.showCoordinates).toBe(E2E.DATA.PREFERENCES.showCoordinates);
    expect(restoredState?.user?.preferences?.animationSpeed).toBe(E2E.DATA.PREFERENCES.animationSpeed);
    
    logger.info(E2E.MESSAGES.SUCCESS.PREFERENCES_PERSISTED, { preferences: restoredState?.user?.preferences });
  });
});