...
export function createMockRouter(router?: Partial<NextRouter>): NextRouter {
  ...
  const parseUrl = (url: string): { pathname: string; query: Record<string, string | string[]> } => {
    try {
  ...
  const mockRouter: NextRouter = {
    ...
    get basePath() { return internalState.basePath; },
    get pathname() { return internalState.pathname; },
    get route() { return internalState.route; },
    get query() { return internalState.query; },
    get asPath() { return internalState.asPath; },
    ...
    reload: jest.fn(() => {
      ...
      (mockRouter.events.emit as jest.Mock)('routeChangeStart', internalState.asPath);
      (mockRouter.events.emit as jest.Mock)('routeChangeComplete', internalState.asPath);
    }),
    ...
    get locale() { return internalState.locale; },
    get locales() { return internalState.locales; },
    get defaultLocale() { return internalState.defaultLocale; },
    
    // State properties - getters for stateful behavior
    get isFallback() { return internalState.isFallback; },
    get isLocaleDomain() { return internalState.isLocaleDomain; },
    get isReady() { return internalState.isReady; },
    get isPreview() { return internalState.isPreview; },
...
export const createMockRouterScenarios = {
   ...
   */
  trainingPage: (id: string) => createMockRouter({
    pathname: '/train/[id]',
    route: '/train/[id]',
    query: { id },
    asPath: `/train/${id}`,
   ...
   * Router with locale
   */
  withLocale: (locale: string) => createMockRouter({
    locale,
    asPath: `/${locale}`,
  }),
};
