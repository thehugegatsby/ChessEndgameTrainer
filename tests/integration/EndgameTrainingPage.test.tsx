...
// Mock TablebaseService - uses central mock  
jest.mock('@shared/services/TablebaseService');
import mockTablebaseService from '../__mocks__/tablebaseService';
...
describe('EndgameTrainingPage Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    act(() => {
      ...
      state.training.setNavigationPositions(
        { id: 1, title: 'Next Position' } as any,
        null
      );
    ...
    // Setup TablebaseService mock defaults
    mockTablebaseService.getEvaluation.mockResolvedValue({
      ...
      evaluation: {
        ...
        dtm: 5,
        moves: [
          { uci: 'e2e3', san: 'Ke3', dtz: 8, dtm: 4, category: 'win' },
        ],
      },
    });
  });
  ...
  describe('Initial Rendering', () => {
    it('should render all main components correctly', () => {
      ...
      // Check title
      expect(screen.getByTestId('position-title')).toHaveTextContent('König gegen König');
      ...
      expect(screen.getByTestId('toggle-analysis')).toHaveTextContent('Analyse AN');
  ...
  describe('Analysis Panel Integration', () => {
    it('should toggle analysis panel and fetch tablebase data', async () => {
      ...
      expect(screen.getByTestId('toggle-analysis')).toHaveTextContent('Analyse AUS');

      // Verify tablebase service was called
      await waitFor(() => {
        expect(mockTablebaseService.getEvaluation).toHaveBeenCalledWith(mockPosition.fen);
      });
    });
    ...
    it('should handle tablebase API errors gracefully', async () => {
      ...
      mockTablebaseService.getEvaluation.mockRejectedValueOnce(
        new Error('Tablebase API unavailable')
      );
      ...
      await waitFor(() => {
        expect(screen.getByText(/Analyse konnte nicht geladen werden/i)).toBeInTheDocument();
      });
    });
    ...
    it('should show loading state while fetching tablebase data', async () => {
      ...
      mockTablebaseService.getEvaluation.mockImplementation(
        () => new Promise(resolve => setTimeout(() => resolve({
          isAvailable: true,
          evaluation: { category: 'win', dtz: 10 },
        }), 1000))
      );
      ...
      await waitFor(() => {
        expect(screen.queryByText(/Lade Analyse.../i)).not.toBeInTheDocument();
      }, { timeout: 2000 });
    });
  });
  ...
  describe('External Links', () => {
    ...
    it('should include PGN in Lichess URL when moves are made', () => {
      ...
      const lichessLink = screen.getByText('Auf Lichess analysieren →');
      expect(lichessLink).toHaveAttribute('href', expect.stringContaining('/pgn/'));
    });
  });
