...
const MOCK_PERFORMANCE_DURATION = ENGINE.EVALUATION_TIMEOUT / 5; // 600ms
...
describe('WebPlatformService', () => {
  ...
  describe('Storage Service', () => {
    ...
    it('should load data from localStorage', async () => {
      ...
      expect(mockLocalStorage.getItem).toHaveBeenCalledWith(`${STORAGE.PREFIX}test-key`);
    ...
    it('should remove data from localStorage', async () => {
      ...
      expect(mockLocalStorage.removeItem).toHaveBeenCalledWith(`${STORAGE.PREFIX}test-key`);
    });
    ...
    it('should clear all chess trainer data', async () => {
      ...
      expect(mockLocalStorage.removeItem).toHaveBeenCalledWith(`${STORAGE.PREFIX}key1`);
      expect(mockLocalStorage.removeItem).toHaveBeenCalledWith(`${STORAGE.PREFIX}key2`);
  ...
  describe('Share Service', () => {
    ...
    it('should throw error when share API not supported', async () => {
      ...
      await expect(newService.share.share({ title: 'test' }))
  ...
  describe('Jest 30 Compatibility', () => {
    ...
    it('should handle error cases properly', async () => {
      ...
      await expect(service.storage.save('test', { data: 'large' }))
