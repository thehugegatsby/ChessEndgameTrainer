/**
 * @fileoverview Tests for TestApiService - Clean Store-Based Architecture
...
describe('TestApiService - Store-Based Architecture', () => {
  ...
  describe('makeMove', () => {
    ...
    it('should make move with dash notation', async () => {
      ...
      expect(mockStoreAccess.makeMove).toHaveBeenCalledWith({ from: 'e2', to: 'e4' });
      expect(result.success).toBe(true);
      expect(result.resultingFen).toBe('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1');
  ...
  describe('triggerEngineAnalysis', () => {
    ...
    it('should handle timeout', async () => {
      ...
      mockStoreAccess.getState.mockReturnValue({
        training: { engineStatus: 'initializing' },
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('Engine analysis timeout after', 100, 'ms');
    });
    ...
    it('should handle errors', async () => {
      ...
      expect(result).toBe(false);
      expect(consoleErrorSpy).toHaveBeenCalledWith('Engine analysis check failed:', expect.any(Error));
    });
  });
